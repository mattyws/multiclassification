...
def filter_features(files_list, events_ids, manager_queue=None):
    # TODO 2: paralel processes
    all_ids = []
    ...
    for f in files_list:
        ...
        for key in events_ids.keys():
            ...
            new_features[key] = patient_events[
                [x for x in ids_in_patients if x != 'Unnamed: 0' and int(x) in events_ids[key]]]
            if new_features[key].empty:
                ...
                data_statistic[key]["missing_events"] += len(new_features[key])
            else:
        ...
        patient_events = patient_events.drop(
            columns=["gcs_verbal", "gcs_motor", "gcs_eyes", "diastolic_blood_pressure"])
