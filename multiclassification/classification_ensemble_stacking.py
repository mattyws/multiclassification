...
notes_textual_representation_path = training_directory + parameters['notes_textual_representation_directory']
...
for trainIndex, testIndex in kf.split(structured_data, classes):
    ...
    start = datetime.datetime.now()
    #TODO: salvar modelos fracos e verificar se já existem
...
structured_predictions = pd.merge(structured_predictions, data_csv[['episode', 'icustay_id']], left_index=True,
                                  right_on='episode', how="left")
structured_representations = pd.merge(structured_representations, data_csv[['episode', 'icustay_id']], left_index=True,
                                  right_on=['episode'], how="left")
# textual_predictions = pd.merge(textual_predictions, data_csv[['episode', 'icustay_id']], left_index=True,
#                                   right_on=['episode'], how="left")
# textual_representations = pd.merge(textual_representations, data_csv[['episode', 'icustay_id']], left_index=True,
#                                   right_on=['episode'], how="left")
...
meta_data_predictions_structured = pandas.merge(meta_data_predictions_structured, data_csv[['episode', 'label']],
                                                left_index=True, right_on="icustay_id", how="left")
...
columns = [c for c in meta_data_predictions_structured.columns if 'label' not in c and 'icustay' not in c]
columns_evaluation = [c for c in meta_evaluation_predictions_structured.columns if 'label' not in c and 'icustay' not in c]
print(columns)
...
columns = [c for c in meta_data_predictions.columns if 'label' not in c and 'icustay' not in c]
columns_evaluation = [c for c in meta_data_evaluation.columns if 'label' not in c and 'icustay' not in c]
...
meta_data_representations_structured = pandas.merge(meta_data_representations_structured, data_csv[['episode', 'label']],
                                                left_index=True, right_on="episode", how="left")
meta_evaluation_representations_structured = pandas.merge(meta_evaluation_representations_structured,
                                                          data_csv[['episode', 'label']],
                                                left_index=True, right_on="episode", how="left")
print(meta_data_representations_structured.columns)
...
columns = [c for c in meta_data_representations_structured.columns if 'label' not in c and 'icustay' not in c]
columns_evaluation = [c for c in meta_evaluation_representations_structured.columns if 'label' not in c and 'icustay' not in c]
print(columns)
print(meta_data_representations_structured.columns)
...
all_representations = pd.merge(structured_representations, textual_representations,
                               left_index=True, right_index=True, how="left")
...
meta_evaluation_representations = pandas.merge(all_evaluations, meta_data_extra, left_on="icustay_id",
                                                right_on="icustay_id", how="left")
meta_data_representations = pandas.merge(meta_data_representations, data_csv[['episode', 'label']],
                                                left_index=True, right_on="episode")
meta_evaluation_representations = pandas.merge(meta_evaluation_representations, data_csv[['episode', 'label']],
                                                left_index=True, right_on="episode")
...
columns = [c for c in meta_data_representations.columns if 'label' not in c and 'icustay' not in c]
columns_evaluation = [c for c in meta_evaluation_representations.columns if 'label' not in c and 'icustay' not in c]
print(columns)
print(meta_data_representations.columns)
